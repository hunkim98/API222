...
```{r}
# '-' means we will drop column 15 and column 2
# Mind the order of the dropping columns! (15 first!)
...
```{r}
# Private is consisted of "No" and "Yes"
# We subtract 1 to convert to 0/1
...
```{r}
# sample gets vector of numbers from 1 to nrow(college_data) and takes 20% of them
# sample first argument is the vector of numbers to sample from
# sample second argument is the number of random numbers to draw
...
```{r}
# %in% is a function that checks if the elements of the first vector are in the second vector
...
```{r}
# the -1 at the end of the training_data and test_data means we are excluding the "first column"
# this allows us to exclude the Private column
# we will use Private as the outcome variable
...
# the return value of which is a vector of indices
accuracy1  <- length(which(knn_model1 == test_data$Private)) / nrow(test_data)
# OR
accuracy1 <- sum(knn_model1 == test_data$Private) / nrow(test_data)
...
```{r}
# the ~ means regress Enroll on all other variables
# the . means all other variables

enroll_reg <- lm(Enroll ~ ., training_data)
# enroll_reg <- lm(Enroll ~ Private, training_data)
...
```{r}
# remove "Enroll" from the test data
predicted_enroll <- predict(enroll_reg, test_data[, -4])
print(predicted_enroll)
```
