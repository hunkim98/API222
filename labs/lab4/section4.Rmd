...
# 333 out of 9667 is default. Only 3% of our data is defaulters.
# If we have a model that returns No everytime, then our model accuracy is 97%
# Probably more costly to wrongly predict a Yes than predict a No
```
...
## setdiff returns the elements of the first vector that are not in the second vector
...
```{r}
# default is our y variable, and student, balance, and income are our x variables
# . means use all other variables in the data frame
# logistic_default is a model
logistic_default <- glm(default ~ student + balance + income,
                        family = binomial, # we added binomial to specify logistic regression
                        # if we do not have this, R will default to linear regression
                        data = train_data)
```
...
logistic_predict <-predict(logistic_default,
                           ...
                           type = "response")
hist(logistic_predict) # most of them recorded as 0 since there are few defaulters
```
...
```{r}
class_predictions <- as.numeric(logistic_predict > 0.5) # threshold is 0.5
logistic_accuracy <- mean(class_predictions == test_data$default)
print(logistic_accuracy)
# but remember that 3% of the data is default, so if we predict No everytime, we will have 97% accuracy
# thus even though the mean is accuracy 97%, it is not a good measure of performance
# we need true positive and true negative accuracy
```
...
# true positive: 0.33% of the true defaulters were predicted to default
```
...
# true negative: 99.4% of the true non-defaulters were predicted to not default
# if I care about more about the true negative accuracy (=the defaulters that should be predicted as defaulters), I can set this true negative accuracy as my criterion
```
...
# we will create a vector of threshold values from 0 to 0.5 in increments of 0.01
# we will test for 0, 0.01, 0.02, ..., 0.5 and then try to choose the optimal threshold
...
```{r}
# initialize error_rates vector to store the true positive and true negative accuracy for each threshold
...
```{r}
library(MASS) # library for QDA and LDA
```
...
# The problem of LDA or QDA is that we cannot set a threshold like we did with logistic regression
```
