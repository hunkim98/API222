...
```{r}
# k_seq means the sequence of k values to test (k is the number of neighbors)
cross_validation_KNN  <- function(data_x, data_y, k_seq, kfolds) {
  ...
  fold_ids <- rep(seq(kfolds), ceiling(nrow(data_x) / kfolds)) # this is just creating tags
  fold_ids <- fold_ids[1:nrow(data_x)]
  
  ## To make the IDs random, randomly rearrange the vector
  fold_ids <- sample(fold_ids, length(fold_ids)) # randomize
...
```{r}
mean_cv_error5 <- rowMeans(knn_cv_error5) # gettin mean of each row
