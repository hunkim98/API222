...
# Remove Sales column
carseat_data <- carseat_data[, -which(names(carseat_data) == "Sales")]
```
...
```{r}
?tree
# high_sales is our target variable
...
deep_tree_preds <- predict(carseats_tree, carseat_data[test,], 
                           # the default is type = "response"
                           # "response" is for continuous outcomes
                           # we want to predict a class
                           type = "class")
```
...
# residual mean deviance: How imbalanced the classes are on the terminal nodes
# If the tree predicted based on data No 6, Yes 4, then the residual mean deviance will be big
```
...
set.seed(222)
# we are pruning
# since this is classification we use prune.misclass
# if it was regression we would use prune.tree
cv_carseats_tree  <- cv.tree(carseats_tree, FUN = prune.misclass)
names(cv_carseats_tree)

# for size tree 19 dev is 50, k is -Inf(overfit)
# we are trying to find an optimal size
# since the lowest deviance happens in size 14 and 7, we can use them
cv_carseats_tree
```
...
# Why it choosed 19 is because it is the smallest size with the lowest deviance
# but it is not the best because it overfit
print(opt_size)

# This is hard coded by Jacob
opt_size <- 7
```
...
```{r}
# we will use cv to prune our tree
...
# randomForest bootstrap the variables
rf.boston <- randomForest(medv ~ ., data = data.frame(boston_data[-test,]), 
                          # importance is defaulted to FALSE
                          # importance TRUE means keep track of variable importance
                          # use 5000 trees
...
# bagging
# bagging is the same thing but you use all variables
...
boost.boston <- gbm(medv ~ ., data = data.frame(boston_data[-test,]), 
                    # default distribution is bernoulli
                    # bernoulli is for classification of 0 and 1
                    # we need to change to gaussian for regression
